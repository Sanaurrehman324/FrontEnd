...
const CreateCategory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post("https://backend-production-8ea6.up.railway.app/api/category/create-category", { name });
      if (data?.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://backend-production-8ea6.up.railway.app/api/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(`https://backend-production-8ea6.up.railway.app/api/category/delete-category/${pId}`);
      if (data.success) {
